function [H,S]=sobi_cp3(X,N,tau,tol,Niter)
%SOBI_CP3 Second Order Blind Identification via Candecomp/Parafac order-3 (CP3)
%       [H,S]=sobi_cp3(X,N,tau,tol,Niter)
% Consider the linear instantaneous Blind Source Separation (BSS) model
%               X = H*S
% where
% - M is the number of sensors
% - N is the number of sources
% - P is the number of samples
% - X (MxP) is the observed is the observed matrix
% - H (MxN) is the unknown mixing matrix
% - S (NxP) is the unknown source matrix
%
% [H,S]=sobi_cp3(X,N) computes a set of K covariance matrices Rx(1),...,Rx(K)
% of the observed signals.
% Then, the mixing matrix is estimated by joint-approximate-diagonalization (JAD)
% of this set of matrices. The JAD problem is solved by computing the CP3 decomposition
% of the tensor built by stacking the matrices along the third dimension:
%       Rx(1) = H1 * Rs(1) * H2.'
%            ...
%       Rx(K) = H1 * Rs(K) * H2.'
% where the source covariance matrices Rs(1),...,Rs(K) are assumed diagonal.
% Note that CP3 yields 2 estimates of H (H1 and H2) since symmetry is ignored
% (in this function). The final estimate of H can be set to H1 or H2.
%
% The algorithm used to fit the cp3 model is the Alternating Least Squares algorithm
% combined with Exact Line Search to speed up convergence (ALS+ELS). 
% Other algorithms can also be used to fit the cp3 model.
%
% Important note: this algorithm works in the over-determined case (M>N), the
% exactly-determined case (M=N) and several under-determined cases (M<N).
%
% Inputs:
% - X : observed matrix
% - N : number of sources
...
% Optional inputs:
% - tau (default 20) : 
%      * if tau is a scalar, it is the number of covariance matrices 
%        computed with a time-shift of one sample only
%      * if tau is a vector, it corresponds to the set of time-shift (in number of samples) 
%        used to compute the covariance matrices. The number of these matrices is length(tau). 
%   So tau=5 is equivalent to tau=[0 1 2 3 4] 
%   and tau=[0 10 20 30], produces 4 matrices with the associated delay values.
% - tol   (default 1e-8) : tolerance to stop the algorithm
% - Niter (default 1000)  : maximum number of iterations
%
% Outputs:
% - H : estimate of the mixing matrix
% - S : * if N<=M, the estimate of the source matrix S is computed by S=pinv(H)*X
%       * if N>M,  S=[], because H is not left pseudo-invertible. 
%       Then, the source matrix has to be recovered in another way,e.g., by
%       combining the estimate of H with a sparsity-based approach (if sources are sparse)
...
%---------------------------------------------------------------------
if exist('tau')~=1 || isempty(tau); tau=20; end
if exist('tol')~=1 || isempty(tol); tol=1e-8; end
if exist('Niter')~=1 || isempty(Niter); Niter=2000; end
[M,P]=size(X);

%-------------------------------------------------------------------
% STEP 1 : compute covariance matrices
%-------------------------------------------------------------------
    Rx = corrX(X,tau);        % tensor holding the set of covariance matrices
    K  = size(Rx,3);          % number of covariance matrices
    
%---------------------------------------------------------------------------------------------------
% Deal with the case where only X is provided in input (e.g. when the number of sources is unknown)
%---------------------------------------------------------------------------------------------------
    if nargin==1
       Rxmat   = reshape(Rx, M^2, K);      
       singval = svd(Rxmat);
       disp('Please enter the number of sources by inspection of the following singular values');
       singval
       N=input('Which rank do you choose?  ');
    end 
    while isempty(N)
       N = input(['Please enter the number of sources','\n']);
    end
    if N*(N-1) > M*(M-1)*M*(M-1)/2
        error('Error: N has to bounded such that N*(N-1) <= M*(M-1)*M*(M-1)/2')	
    end
    if N>K
        error('Error: N should not be greater than the number of covariance matrices')	
    end
    
%----------------------------------------------------------------------------------
% STEP 2 : compute PARAFAC decomposition of Rx
%----------------------------------------------------------------------------------
if M >= N   
  % over or exactly determined case 
  % use 1 initialization, method 'elsr' and compression  
  [H] = cp3_alsls(Rx,N,'elsr','on',tol,Niter,[],[],1); 

else
    % under-determined case: use 3 initializations because the optimization
    % problem is harder to solve
  [H] = cp3_alsls(Rx,N,'elsr','on',tol,Niter,[],[],3); 
end
    
%----------------------------------------------------------------------------------
% STEP 3 : Estimate source signal in the (over)-determined case
%----------------------------------------------------------------------------------
     S=[];
     if M>=N
        S=pinv(H)*X;
...
% Computation of a set of covariance matrices
    %---------------------------------------------------------------------------------------------
    % Inputs:
    % X (MxP)             :  matrix holding P samples of M signals
    % tau (default 2M)     : 
    %      * if tau is a scalar, it corresponds to the number of covariance 
    %        matrices that are computed with a time-shift of one sample only
    %      * if tau is a vector, it corresponds to the set of time-shift (in number of samples)
    %       used to compute the covariance matrices. The number of these matrices is length(tau)
    %      So tau=5 is equivalent to tau=[0 1 2 3 4]. 
    %      So tau=[0 10 20 30], produces 4 matrices with the associated delay values
    %----------------------------------------------------------------------------------------------
    % Output:
    % Rx (MxMxK)        : three-way array holding the computed set of K covariance matrices
    %----------------------------------------------------------------------------------------------
    [M,P] = size(X);
...
%-----------------------------------------------------------------------
%  Deal with initialization in input parameters
%-----------------------------------------------------------------------
...
for ninit=1:Ninit   % Try Ninit different initializations
    ...
    while stop==0
       ...
       % Perform ALS updates
        ...
        % Calculate the new phi to the Model and decide to stop or not
        phi=norm(X3-kr(C,B)*A.','fro')^2;
        if nargout==7
           phi_vec=[phi_vec, phi]; 
        end
...
%      - X: IxKJ matrix unfolding of the observed tensor X 
